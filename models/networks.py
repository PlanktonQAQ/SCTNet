...
def IRN(input_nc, output_nc, ngf, which_model_netG, norm='none', use_dropout=False, init_type='normal',
        gpu_ids=[]):  # Batch norm -> nonw
    IRN = None

    IRN = HISTUnet3_Res(input_nc, output_nc, ngf, norm_layer=get_norm_layer(norm_type=norm),
                        use_dropout=use_dropout).cuda()
...
def HEN(input_nc, output_nc, ngf, which_model_netC, norm='batch', use_dropout=False, init_type='normal', gpu_ids=[]):
    ...
    HEN = ConditionNetwork2(input_nc, output_nc, ngf, norm_layer=get_norm_layer(norm_type=norm),
                            use_dropout=use_dropout, gpu_ids=gpu_ids).cuda()
