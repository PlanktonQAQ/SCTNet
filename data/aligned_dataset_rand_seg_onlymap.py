...
class AlignedDataset_Rand_Seg_onlymap(BaseDataset):
    def initialize(self, opt):
        ...
        # self.dir_A = '/media/x3022/42B0CAB7B0CAB0A9/image_color_transfer/colorTransferwork' \
        #              '/Color_Transfer_Histogram_Analogy/dataset/input/'
        # self.dir_B = '/media/x3022/42B0CAB7B0CAB0A9/image_color_transfer/colorTransferwork' \
        #              '/Color_Transfer_Histogram_Analogy/dataset/target/'
        # self.dir_A_Map ='/media/x3022/42B0CAB7B0CAB0A9/image_color_transfer/colorTransferwork' \
        #                 '/Color_Transfer_Histogram_Analogy/dataset/seg_in/'
        # self.dir_B_Map = '/media/x3022/42B0CAB7B0CAB0A9/image_color_transfer/colorTransferwork' \
        #                  '/Color_Transfer_Histogram_Analogy/dataset/seg_tar/'
    ...
    def __getitem__(self, index):
        ...
        A_img = Image.open(A_path).convert('RGB')
        B_img = Image.open(B_path).convert('RGB')
        A_img_b = Image.open(A_path_map).convert('RGB')
        B_img_b = Image.open(B_path_map).convert('RGB')

        A = self.transform_type(A_img)
        B = self.transform_type(B_img)
        A_map = self.transform_type(A_img_b)
        B_map = self.transform_type(B_img_b)
